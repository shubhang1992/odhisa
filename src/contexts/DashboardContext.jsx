...
function buildSelectedIds(level, entityId) {
  ...
  while (currentLevel && currentLevel !== 'country' && currentId) {
    ...
    if (!entity?.parentId || entity.parentId === 'od') break;
...
export function DashboardProvider({ children }) {
  ...
  const drillUp = useCallback((fromLevel) => {
    ...
    let parentLevel = PARENT_LEVEL[level];
    // Odisha model: skip the synthetic region step when drilling up from district.
    if (parentLevel === 'region') parentLevel = 'country';
