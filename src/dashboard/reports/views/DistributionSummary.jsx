...
export default function DistributionSummary({ onBack }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => formatINR(row.metrics?.aum || 0),
    },
    {
  ...
  const description = useMemo(() => {
    ...
    return `${m.totalSubscribers?.toLocaleString()} subscribers \u00B7 ${m.totalBranches?.toLocaleString()} branches \u00B7 ${formatINR(m.aum)} AUM`;
  }, [country]);
