                  ...
                  <motion.div key="home" {...viewAnim}>
                    ...
                    <div className={styles.overviewHero}>
                      <button className={styles.overviewTotal} onClick={() => goAgents(null)}>
                        ...
                        <div className={styles.overviewTotalAmount}>{formatINR(summary?.totalCommissions || 0)}</div>
                        ...
                        {!readOnly && editingRate ? (
                          <span className={styles.rateEditRow}>
                            <input
                              ...
                              aria-label="Commission rate in INR"
                              autoFocus
                            />
                          ...
                          <>
                            <span className={styles.rateInlineValue}>{formatINR(rate || 0)} per subscriber</span>
                            {!readOnly && (
                    ...
                    <div className={styles.attentionSection}>
                      ...
                      <button className={styles.attentionRow} data-type="disputed" onClick={goDisputed}>
                        ...
                        <div className={styles.attentionInfo}>
                          ...
                          <div className={styles.attentionDesc}>{formatINR(summary?.totalDisputed || 0)} across {scopedDisputedAgents.length} agents</div>
                        </div>
                      ...
                      <button className={styles.attentionRow} data-type="requests" onClick={goRequests}>
                        ...
                        <div className={styles.attentionInfo}>
                          ...
                          <div className={styles.attentionDesc}>{formatINR(summary?.totalRequested || 0)} from {scopedRequestAgents.length} agents</div>
                        </div>
                ...
                {view === 'agent-detail' && agentDetail && (
                  <motion.div key="agent-detail" {...viewAnim}>
                    ...
                    <div className={styles.section}>
                      <div className={styles.sectionHeader}>
                        <div>
                          ...
                          <span className={styles.sectionCount}> — {formatINR(agentDetail.totalPaid)}</span>
                        </div>
                      </div>
                      {agentDetail.paidTransactions.slice(0, 5).map((tx) => (
                        <div key={tx.id} className={styles.txRow}>
                          ...
                          <div className={styles.txAmount} data-status="paid">{formatINR(tx.amount)}</div>
                    ...
                    <div className={styles.section}>
                      <div className={styles.sectionHeader}>
                        <div>
                          ...
                          <span className={styles.sectionCount}> — {formatINR(agentDetail.totalDue)}</span>
                        </div>
                      </div>
                      {agentDetail.dueTransactions.slice(0, 5).map((tx) => (
                        <div key={tx.id} className={styles.txRow}>
                          ...
                          <div className={styles.txName}>{tx.subscriberName}</div>
                          <div className={styles.txAmount} data-status="due">{formatINR(tx.amount)}</div>
                    ...
                    {!readOnly && agentDetail.dueTransactions.length > 0 && (
                      ...
                      >
                        {Icons.wallet}
                        Settle Due Commissions ({formatINR(agentDetail.totalDue)})
                      </button>
                    )}
                      ...
                      subscribers.map((sub) => (
                        <div key={sub.subscriberId} className={styles.subRow}>
                          ...
                          <div className={styles.subMeta}>
                            <div className={styles.subAmount}>{formatINR(sub.totalContributions)}</div>
                        ...
                        <div key={agent.agentId} className={styles.selectableRow} data-selected={!readOnly && selectedIds.has(agent.agentId)}>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className={styles.agentAmountLabel}>
                                {formatINR(agent.disputedAmount)}
                              </div>
                            </div>
                          </button>
                        </div>
                        ...
                        {selectedIds.size > 0 && (
                          ...
                          >
                            <div className={styles.floatingInfo}>
                              <span className={styles.floatingCount}>{selectedIds.size}</span> agents — {formatINR(selectedTotal)}
                            </div>
                    ...
                    <div className={styles.section}>
                      ...
                      {selectedDisputeAgent.disputes.map((d) => (
                        <div key={d.id} className={styles.txRow}>
                          ...
                          <div className={styles.txAmount} style={{ color: 'var(--color-status-poor)' }}>{formatINR(d.amount)}</div>
                          {!readOnly && (
                            <div className={styles.txActions}>
                        ...
                        <div key={agent.agentId} className={styles.selectableRow} data-selected={!readOnly && selectedIds.has(agent.agentId)}>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className={styles.agentAmountLabel}>
                                {formatINR(agent.requestedAmount)}
                              </div>
                            </div>
                          </button>
                        </div>
                    ...
                    <div className={styles.section}>
                      ...
                      {selectedRequestAgent.requests.map((r) => (
                        <div key={r.id} className={styles.txRow}>
                          ...
                          <div className={styles.txAmount} data-status="due">{formatINR(r.amount)}</div>
                          <div className={styles.txActions}>
                    ...
                    <div className={styles.modalBody}>
                      <div className={styles.modalSummary}>
                        ...
                        <div className={styles.modalSummaryValue}>
                          {formatINR(settleScope === 'agent' ? (agentDetail?.totalDue || 0) : (summary?.totalDue || 0))}
                        </div>
                      </div>
                      <div className={styles.modalSummary}>
