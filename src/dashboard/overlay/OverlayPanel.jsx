...
import { formatINR, EASE_OUT_EXPO as EASE } from '../../utils/finance';
    ...
    <div className={styles.globalSearch}>
      <div className={styles.searchBarCompact}>
        ...
        <input
          ...
          type="text"
          placeholder="District, branch, agent…"
          value={query}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <MetricRow
            ...
            value={formatINR(d.contrib)} label="Contributions"
          ...
          <MetricRow
            ...
            value={formatINR(d.withdraw)} label="Withdrawals"
          ...
          {topBranch && (
            <MetricRow
              ...
              value={topBranch.name} label={`Top Branch · ${formatINR(topBranch.contribution)}`}
...
export default function OverlayPanel() {
  ...
  const parentId = level === 'country' ? 'od' : selectedIds[level];
  // Odisha model: skip the region step and show districts directly under the state.
  const nextLevel = level === 'country' ? 'district' : (CHILD_LEVEL[level] || null);
  const { data: currentEntity } = useCurrentEntity(level, selectedIds);
  const { data: directChildren = [] } = useChildren(level, parentId);
  const { data: allDistricts = [] } = useAllEntities('district');
  const children = level === 'country' ? allDistricts : directChildren;
    ...
    >
      ...
      <div className={styles.panelHeader}>
        {level === 'country' ? (
          <div className={styles.titleBlock}>
            <span className={styles.stateTag}>Odisha</span>
      ...
      <div className={styles.panelBody}>
          ...
          <div className={styles.aumBlock}>
            <span className={styles.aumValue}>{formatINR(aum)}</span>
          ...
          <div className={styles.quickStats}>
            <div className={styles.stat}>
              <span className={styles.statNum}>{formatINR(metrics.totalContributions)}</span>
              <span className={styles.statLabel}>Contributions</span>
            </div>
            <div className={styles.statDivider} />
            <div className={styles.stat}>
              <span className={styles.statNum}>{formatINR(metrics.totalWithdrawals)}</span>
            ...
            <button className={styles.commissionBlock} onClick={() => setCommissionsOpen(true)}>
              ...
              <div className={styles.commissionStats}>
                <span className={styles.commissionStat}>
                  ...
                  {formatINR(commissionSummary.totalPaid)}
                </span>
                <span className={styles.commissionStat}>
                  <span className={styles.commissionDot} data-status="due" />
                  {formatINR(commissionSummary.totalDue)}
                </span>
                {commissionSummary.countDisputed > 0 && (
